    ...
    public void clientMenu(){
        try {
            ...
            while (true){
                ...
                if(headerReader == ClientMessage.PROTOCOL_LOGIN_RESPONSE){
                    ...
                    dos.flush();
                }else if(headerReader == ClientMessage.PROTOCOL_LOGIN_SUC){
                    System.out.println(ClientMessage.LOGIN_SUC); // 로그인 성공
                    dis.read(receivePacket);
                    group_id = changeByteToInt(receivePacket); // group_id에 따라 개인 메뉴로 이동
                    break;
                }else if(headerReader == ClientMessage.PROTOCOL_LOGIN_FAIL){
                    System.out.println(ClientMessage.LOGIN_FAIL); // 로그인 실패 종료
                    return;
                }
            }
            switch (group_id){
            ...
            while(true){
                ...
                switch (command){
                    ...
                    case ClientMessage.INPUT_COMMAND_4:
                        ...
                        if (headerReader == ClientMessage.PROTOCOL_ADMIN_UPDT_NOT_OPEN_SUB) { // 비개설교과목 id 존재 -> 수정진행
                            while (true) {
                                ...
                                switch (command) {
                                    case 1:
                                        System.out.print(ClientMessage.UPDT_NOT_OPEN_SUB_NAME_REQ);
                                        name = sc.next();
                                        protocol.getAdminRequestPacket(Message.FUNCTION_TYPE_UPDATE_SUBJECT, Message.CODE_TYPE_2);
                                        protocol.setStrToByte(name);
                                        break;
                                    case 2:
                                        System.out.print(ClientMessage.UPDT_NOT_OPEN_SUB_GRADE_REQ);
                                        possible_grade = sc.nextInt();
                            ...
                            while (true) {
                                ...
                                switch (command){
                                    ...
                                    case ClientMessage.INPUT_COMMAND_5:
                                        ...
                                        break;
                                    default:
                                        System.out.println(ClientMessage.UPDT_OPEN_SUB_INPUT_FAIL);
                                        continue;
                                }
                                dos.write(protocol.getPacket(), 0, protocol.getSize());
                                dos.flush();
                    ...
                    case ClientMessage.INPUT_COMMAND_13:
                        ...
                        break;
                    case ClientMessage.INPUT_COMMAND_14:
            ...
            while(true) {
                ...
                switch (command){
                    case ClientMessage.INPUT_COMMAND_1:     // 교수 개인정보 수정   | 전공 변경이 없어서 CodeType에 5번 추가함
                        ...
                        } else if(headerReader == ClientMessage.PROTOCOL_PROFESSOR_UPDATE_INFO_FAIL){                        // 교수 개인정보 수정 성공
                    ...
                    case ClientMessage.INPUT_COMMAND_4:     // 교과목 목록 조회
                        ...
                        dis.read(receivePacket);
                        headerReader = readHeader(receivePacket);
                        if (headerReader == ClientMessage.PROTOCOL_PROFESSOR_SUBJECT_LIST) {
                            dis.read(receivePacket);
                ...
                switch (command){
                    ...
                    case ClientMessage.INPUT_COMMAND_2:
                        ...
                        while (true) {
                            dis.read(receivePacket);
                            headerReader = readHeader(receivePacket);
                            if (headerReader == ClientMessage.PROTOCOL_STUDENT_ENROLL_SUC) {
                                System.out.println(ClientMessage.ENROLL_SUBJECT_SUC);
                                break;
                            } else if (headerReader == ClientMessage.PROTOCOL_STUDENT_ENROLL_FAIL) {
                    ...
                    case ClientMessage.INPUT_COMMAND_3:
                        ...
                        while (true) {
                            dis.read(receivePacket);
                            headerReader = readHeader(receivePacket);
                            if (headerReader == ClientMessage.PROTOCOL_STUDENT_ENROLL_CANCEL_SUC) {
                                System.out.println(ClientMessage.CANCEL_SUBJECT_SUC);
                                break;
                            } else if (headerReader == ClientMessage.PROTOCOL_STUDENT_ENROLL_CANCEL_FAIL) {
                    ...
                    case ClientMessage.INPUT_COMMAND_5:
                        ...
                        while (true){
                            dis.read(receivePacket);
                            headerReader = readHeader(receivePacket);
                            if (headerReader == ClientMessage.PROTOCOL_STUDENT_SELECT_SUBJECT_PLAN_LIST) {
                                dis.read(receivePacket);
